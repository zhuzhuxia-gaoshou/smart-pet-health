...
- **实用工具**：浏览器到期通知（🔔 开关，新事项弹窗直达宠物）、数据导出（宠物/记录 CSV、AI 会话 .md、数据库备份）、演示数据一键重置（顶栏 ⋯ 菜单）
...
- **AI 助手（LangChain Agent）**：
  ...
  - **对话会话**：历史会话自动保存，AI 助手左侧列表随时回看、点击继续聊（支持"那它的体重呢？"式追问）；单个会话可删除、可重命名，「新对话」另起炉灶不删历史
  - **体重 AI 解读**：详情页体重图旁一键生成趋势判断（升幅是否合理、原因、建议），按数据签名缓存
  - **AI 护理计划**：结合物种习性与记录生成月度计划（到期复做 + 超期补做 + 周期预排），逐项一键转为正式记录
...
| POST | `/api/admin/reset-demo` | 重置演示数据（清空并重建示例，危险操作） |
